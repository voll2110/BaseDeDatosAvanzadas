...
Es un bloque de codigo SQL guardado dentro de la nase de datos (**Un objeto de la DB**) que puede ejecutarse cuando se necesite
...
2. Mejor Rendimiento
3. Mayor Seuridad (Evitar la Inyeccion SQL)
...
GO
```
2️⃣ Prametros en Stored Proceures
...
EXEC spu_cliente_consultarporid2 @Id='ANTONt';

DECLARE @Id2 AS CHAR(10) = (SELECT CustomerID FROM customers WHERE CustomerID = 'ANTON');
EXEC spu_cliente_consultarporid @Id2;

DECLARE @Id3 CHAR(10);
SELECT @Id3=(SELECT CustomerID FROM customers WHERE CustomerID = 'ANTON');
EXEC spu_cliente_consultarporid @Id3;

```
3️⃣ Parametros OUTPUT

Loa valores OUTOUT devuelven valores al usuario
```sql
--Parametros OUTPUT
...
-- Utilizar variable de salida 
DECLARE @res INT;
EXEC spu_operacion_sumar 4,5,@res OUTPUT;
SELECT @res AS [SUMA];
GO
```
4️⃣ Logica dentro del SP
...
GO
```
5️⃣## STORED PROCEDURES CON INSERT, UPDATE Y DELETE
...
```sql
BEGIN TRY
    --Codigo que puede generar el error
END TRY
BEGIN CATCH
...
END CATCH;
```
> NOTA: ERRORESQUE NO CAPTURA EL TRY-CATCH
